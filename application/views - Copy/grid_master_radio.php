...
<script>
	function edit_as_radio(o)
	{
		setTimeout(function(){
			jQuery(o).hide();
			jQuery(o).parent().append('<label><input title="Perbaikan" type="radio" name="rd" value="Perbaikan" /> Zero </label>');
			jQuery(o).parent().append('<label><input title="5" type="radio" name="rd" value="5" /> Five </label>');
			jQuery(o).parent().append('<label><input title="10" type="radio" name="rd" value="10" /> Ten </label>');
			jQuery('input:radio[name="rd"][value="'+parseInt(o.value)+'"]').prop('checked', true);
			jQuery('input[name="rd"]').click(function(){
				jQuery('#total, input[name=total].editable').val(jQuery(this).val());
			});
		},100);
	}
	</script>		
