...
class Ion_auth_model extends CI_Model
{
	...
	public function set_session($user)
	{
		...
		//set user menu privilage
		$gr_menu=array();
		$idg = implode("','",$session_data['groups']);	
		$privilage = $this->ion_auth->get_priv($idg);
		foreach($privilage as $m){
			$val = $m->gr_menu;
			array_push($gr_menu,$val);
		}
		$session_data['priv'] = $gr_menu;
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			// apip merubah line 2093m karena identity_cookie_name tdk ditemukan
			set_cookie(array(
			    // 'name'   => $this->config->item('identity_cookie_name', 'ion_auth'),
				'name'   => $this->config->item('csrf_cookie_name'),
	...
	/*for session menu privilage*/
	public function get_priv($groups){
		$qry = $this->db->query("SELECT DISTINCT(idmenu) as gr_menu FROM app_menu_priv WHERE idgroup IN ('".$groups."')");
		if ($qry->num_rows() > 0){
			return $qry->result();
		}
		return null;
	...
	public function kantorlist()
	{
		...
		$this->db->select('*')->from('ms_lokasi_tvri');
		...
		foreach ($query->result() as $row)
				{
					  $listArray[$row->idlokasi] = $row->nama_lokasi;                
				}               
