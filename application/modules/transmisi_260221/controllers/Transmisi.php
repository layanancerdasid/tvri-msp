...
class Transmisi extends CI_Controller {
	...
	public function pemancar($idparams)
	{
		...
		$g2->select_command = "SELECT * FROM (
			SELECT id_pemancar
			, jenis_transmisi, ms.idlokasi, frekuensi, coverage_populasi, coverage_area, channel_1, aktif1, channel_2, aktif2,channel_3, aktif3, tipe, power_nominal, power_real, created, iduser_created
			, CONCAT(us.first_name,' ',IFNULL(us.last_name,'')) nama_lengkap, keterangan_kerusakan, kondisi, thn_pengadaan, merk, isr
		...
		$col["editoptions"] = array("value"=>"Analog:Analog;Digital:Digital;Dualcast:Dualcast");
		$cols[] = $col;
		...
		$col["editable"] = true;
		$col["edittype"] = "select"; // render as select
		$str = $g2->get_dropdown_values("SELECT  DISTINCT channel AS k, concat('Channel ', channel,' Freq ',frequency)  AS v FROM ms_kanal where aktif='1' ORDER BY channel");
		...
		$col["editable"] = true;
		$col["edittype"] = "select"; // render as select
		$str2 = $g2->get_dropdown_values("SELECT  DISTINCT channel AS k, concat('Channel ', channel,' Freq ',frequency)  AS v FROM ms_kanal where aktif='1' ORDER BY channel");
		...
		$col["editable"] = true;
		$col["edittype"] = "select"; // render as select
		$str3 = $g2->get_dropdown_values("SELECT  DISTINCT channel AS k, concat('Channel ', channel,' Freq ',frequency)  AS v FROM ms_kanal where aktif='1' ORDER BY channel");
	...
	public function catudaya($idparams)
	{
		...
		$g5->select_command = "SELECT * FROM (
			SELECT idcatudaya,ms.idlokasi,sumber_daya,besar_daya,merk,phase,kondisi,thn_perolehan, iduser_created,created, CONCAT(us.first_name,' ',IFNULL(us.last_name,'')) nama_lengkap 
