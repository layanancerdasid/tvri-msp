...
class Master extends CI_Controller {
	...
	public function kabupaten()
	{
		...
		$col["stype"] = "checkbox"; 
		$col["formatter"] = "checkbox";
		$col["editoptions"] = array("value"=>$idprovinsi, "defaultValue"=>$idprovinsi);
		$cols[] = $col;
		...
		$col["editable"] = true;
		$col["width"] = "180";
		$col["editoptions"] = array("maxlength" => "50", "size"=>40);
		$col["editrules"] = array("required"=>true); 
		$cols[] = $col; 
		...
		$col["formatter"] = "currency";
			...
			$col["width"] = "150";
			$col["editrules"] = array("required"=>true);
			$col["editoptions"] = array("onfocus"=>"set_field_number0(this)", "maxlength" => "18", "size"=>20);
		$cols[] = $col; 
	...
	public function kecamatan()
	{
		...
		$col["stype"] = "checkbox"; 
		$col["formatter"] = "checkbox";
		$col["editoptions"] = array("value"=>$idkabupaten, "defaultValue"=>$idkabupaten);
		$cols[] = $col;
	...
	public function kanal()
	{
		...
		$grid["view_options"] = array("width"=>"450");
		$grid["export"] = array("format"=>"xls", "filename"=>$this->file_name, "sheetname"=>"master-kategoriuji");
	...
	function arealayanan() {
		...
		$this->load->view('layout/header',$head);
		$this->load->view('layout/sidebar');
		$this->load->view('grid_master_tag2',$data);
		$this->load->view('layout/footer');
	}
	...
	public function merk()
	{
		...
		$grid["edit_options"] = array("width"=>"550");
		$grid["view_options"] = array("width"=>"550");
		$grid["export"] = array("format"=>"xls", "filename"=>$this->file_name, "sheetname"=>"master_merk");
		...
		$g->select_command = "SELECT idmerk,merk,aktif,created,iduser_created FROM ms_merk";	
		$g->table = "ms_merk";
		...
		$col = array();
		$col["title"] = "Nama Merk Pemancar";
		$col["name"] = "merk";
		$col["editoptions"] = array("maxlength" => "80", "size" =>"40");
		$col["search"] = true;
		$col["editable"] = true;
		$col["editrules"] = array("required"=>true); 
		$col["hidden"] = false;
		...
		$head['title'] = $caps;
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["master","merk"]);
		$data['fluid'] = true;
		$data['gridout'] = $g->render("list1");
	...
	public function pegawai()
	{
		...
		$grid["caption"] = $caps;				
		$grid["rowNum"] = 10; 
		$grid["rowList"] = array(10,20,30,40,'All');
		$grid["rownumbers"] = true;
		$grid["rowList"] = array();$grid["viewrecords"] = true;
		$grid["rownumbers"] = true;
		$grid["sortname"] = 'idpeg'; 
	...
	public function lokasi()
	{
		...
		$grid["multiselect"] = false; 
		$grid["toolbar"] = "both"; 
		$grid["add_options"] = array("recreateForm" => true, "closeAfterEdit"=>true, 'width'=>'650', 'top'=>'10'); 
		$grid["edit_options"] = array("recreateForm" => true, "closeAfterEdit"=>true, 'width'=>'650', 'top'=>'10'); 
		$grid["view_options"] = array("recreateForm" => true, "closeAfterEdit"=>true, 'width'=>'650', 'top'=>'10');
		$grid["export"] = array("format"=>"pdf", "filename"=>"SiteTVRI", "heading"=>"Daftar Lokasi Site/Kantor TVRI", "orientation"=>"landscape", "paper"=>"a4");
		$grid["export"] = array("filename"=>"SiteTVRI", "heading"=>"Daftar Lokasi Site/Kantor TVRI", "orientation"=>"landscape", "paper"=>"F4","range"=>"filtered");
		$grid["export"]["sheetname"] = "TVRI";
		$g->set_actions(array(
				"add"=>true,
				"edit"=>true,
				"delete"=>true,
		...
		$g->select_command = "SELECT ms_lokasi_tvri.idlokasi, ms_lokasi_tvri.idprovinsi, ms_provinsi.nama_provinsi, ms_kabupaten.nama_kabupaten, ms_kecamatan.nama_kec, ms_lokasi_tvri.idkabupaten, ms_lokasi_tvri.idkec, nama_lokasi, longitude, latitude, alamat, status_kantor, pic, wilayah_layanan, telp_pic, ms_lokasi_tvri.kanal, token, path_gedung, path_struktur, (SELECT GROUP_CONCAT(wilayah_layanan SEPARATOR ', ') FROM ms_area_layanan 
		WHERE FIND_IN_SET(id, ms_lokasi_tvri.wilayah_layanan) <> 0 ) as labellayanan,(SELECT GROUP_CONCAT(concat('Ch. ',channel, ' / Freq ', frequency) SEPARATOR ', ') FROM ms_kanal 	
		WHERE FIND_IN_SET(id_kanal, ms_lokasi_tvri.kanal) <> 0 ) as labelkanal, ms_status_lahan.nama_status_lahan
		from ms_lokasi_tvri
		LEFT JOIN ms_provinsi on ms_provinsi.idprovinsi = ms_lokasi_tvri.idprovinsi
		LEFT JOIN ms_kecamatan on ms_kecamatan.idkec = ms_lokasi_tvri.idkec
		LEFT JOIN ms_kabupaten on ms_kabupaten.idkabupaten = ms_lokasi_tvri.idkabupaten
		LEFT JOIN ms_status_lahan on ms_status_lahan.idstatuslahan=ms_lokasi_tvri.status_kantor";
		//LEFT JOIN ms_pegawai on ms_pegawai.idpeg = ms_lokasi_tvri.koordinator";
		$g->table = "ms_lokasi_tvri";
		...
		$col["width"] = "170";
		$col["editrules"] = array("required"=>false);
		$col["editoptions"] = array("maxlength" => "60", "size"=>40);
		$col["show"] = array("list"=>true,"edit"=>true,"add"=>true,"view"=>true);
		$cols[] = $col;
		...
		$col["editable"] = true;
		$col["edittype"] = "select"; // render as select
		$str = $g->get_dropdown_values("select distinct idprovinsi as k, nama_provinsi as v from ms_provinsi where aktif='1' order by 2");
		$col["editoptions"] = array(
										"value"=>":;".$str,
										"onchange" => array(
															"update_field"=>"idkabupaten",
															"sql"=>"select ms_kabupaten.idkabupaten as k, nama_kabupaten as v from ms_kabupaten WHERE idprovinsi = {idprovinsi} order by 2",
															)
									);
		...
		$col["title"] = "Kabupaten";
		$col["name"] = "idkabupaten";
		$col["dbname"] = "ms_lokasi_tvri.idkabupaten";
		$col["width"] = "180";
		$col["search"] = true;
		$col["editable"] = true;
		$col["edittype"] = "select"; // render as select
		$str = $g->get_dropdown_values("select idkabupaten as k, nama_kabupaten as v from ms_kabupaten order by 2");
		$col["editoptions"] = array(
					"value"=>":;".$str,
					"onchange" => array(	"update_field"=>"idkec",
											"sql"=>"select ms_kecamatan.idkec as k, ms_kecamatan.nama_kec as v from ms_kecamatan inner join ms_kabupaten on ms_kecamatan.idkabupaten = ms_kabupaten.idkabupaten WHERE ms_kecamatan.idkabupaten IN ({idkabupaten}) order by 2"
										)
					);
		...
		$col["title"] = "Kecamatan";
		$col["name"] = "idkec";
		$col["dbname"] = "ms_lokasi_tvri.idkec";
		$col["width"] = "180";
		$col["search"] = true;
		$col["editable"] = true;
		$col["edittype"] = "select"; // render as select
		$str = $g->get_dropdown_values("select idkec as k, nama_kec as v from ms_kecamatan order by 2");
		$col["editoptions"] = array(
									"value"=>":;".$str
									);
		...
		/*$col = array();
		$col["title"] = "Koordinator";
		$col["name"] = "koordinator";
