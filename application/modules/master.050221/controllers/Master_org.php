...
class Master extends CI_Controller {
	...
	{
		if(in_array(6,$this->session->userdata('priv')) == FALSE) return redirect('404_override');
	...
	public function satker()
	{
		...
		$g->select_command = "SELECT iddirektorat, nama_direktorat, aktif, parent_id  FROM ms_direktorat where parent_id='0'  ";
	...
	public function bidang()
	{
		...
		$grid["rowList"] = array(20,40,100,'All');
		$grid["rownumbers"] = true;
		$grid["sortname"] = 'parent_id asc, nama_direktorat'; 
		$grid["autowidth"] = true;
		$grid["shrinkToFit"] = true;
		$grid["forceFit"] = false;
		$grid["resizable"] = true;
	...
	public function seksi()
	{
		...
		$direktorat= $_REQUEST['iddirektorat'];
		$nama_bidang = $_REQUEST["nama_direktorat"];
		if($direktorat){
			$caps .= ", Bidang ".$nama_bidang;			
			$filter = "WHERE parent_id = '$direktorat' ";	
			$grid["subGrid"] = false; 
		}
		...
		$g->select_command = "SELECT iddirektorat, nama_direktorat,aktif, parent_id  FROM ms_direktorat $filter  ";
		$g->table = "ms_direktorat";
	...
	public function polarisasi()
	{
		...
		$grid["edit_options"] = array("width"=>"550");
		$grid["view_options"] = array("width"=>"550");
		$grid["export"] = array("format"=>"xls", "filename"=>$this->file_name, "sheetname"=>"master2");
		...
		$head['title'] = $caps;
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["master2","polarisasi"]);
		$data['fluid'] = true;
		$data['gridout'] = $g->render("list1");
	...
	public function plot_pattern()
	{
		...
		$head['title'] = $caps;
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["master2","plot pattern"]);
		$data['fluid'] = true;
		$data['gridout'] = $g->render("list1");
	...
	public function fft_size()
	{
		...
		$head['title'] = $caps;
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["master2","FFT Size"]);
		$data['fluid'] = true;
		$data['gridout'] = $g->render("list1");
	...
	public function modulation()
	{
		...
		$head['collapse'] = false;
		$head['title'] = "Home";
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["Master3","Modulation"]);
		$this->load->view('layout/header',$head);
		$this->load->view('layout/sidebar');
		$this->load->view('grid_master',$data);
		$this->load->view('layout/footer');
	...
	/*	$col = array();
		...
		*/
		...
		$head['title'] = $caps;
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["master2","pegawai"]);
		$data['fluid'] = true;
		$data['gridout'] = $g->render("list1");
	...
	public function lokasi()
	{
		...
		$grid["export"] = array("format"=>"xls", "filename"=>$this->file_name, "sheetname"=>"master3");
		...
		$g->select_command = "SELECT idlokasi, idprovinsi, idkabupaten, idkec, iddesa, nama_lokasi, longitude, latitude, alamat, status_kantor, frekuensi, wilayah_layanan, telp_pic, email, token, path_gedung, path_struktur, iduser_created, (SELECT GROUP_CONCAT(wilayah_layanan SEPARATOR ', ') FROM ms_area_layanan WHERE FIND_IN_SET(id, ms_lokasi_tvri.wilayah_layanan) <> 0 ) as labellayanan from ms_lokasi_tvri";
		$g->table = "ms_lokasi_tvri";
		...
		$col["isnull"] = true;
		$col["width"] = "200";
		$col["editoptions"] = array("maxlength" => "30","size"=>30, "placeholder"=>"...@email.com");
