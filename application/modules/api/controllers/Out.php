...
class Out extends Base_Controller{
    ...
    public function laporan_digital_post(){
        ...
        if (!empty($_FILES['image']['name'])) {
            log_message('info', 'uploading image...');
			...
			if(!$this->upload->do_upload('image')){
				...
				$this->response(0, REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
            }
            log_message('info', 'File info '. json_encode($this->upload->data()));
    ...
    public function laporan_trxbamux_post(){
        ...
        //idqc label for personel
        $label = "";
        if($idqc == '40' || $idqc == '44'){
            $idpeg = $post['jawaban'];
            $teknisi_tvri = $this->db->query("SELECT nama_lengkap FROM ms_pegawai WHERE idpeg='$idpeg'")->row_array();
            $label = $teknisi_tvri['nama_lengkap'];
        }
        $data = [
            'idlokasi' => $post['idlokasi'],
