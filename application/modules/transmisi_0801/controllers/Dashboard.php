	...
	public function load_lokasi()
	{
		...
		if ($rsl)
			foreach ($rsl as $idx => $v) {
				$arrloc[] = array(
					"idlokasi" => $v->idlokasi, "nama_lokasi" => $v->nama_lokasi, "lat" => $v->latitude, "long" => $v->longitude, "alamat" => $v->alamat, "jenis_kantor" => $v->nama, "status_lahan" => $v->nama_status_lahan, "transmisi" => $v->tipe_transmisi
				);
