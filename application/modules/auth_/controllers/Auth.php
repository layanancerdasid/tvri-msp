...
class Auth extends CI_Controller
{
	...
	public function users(){
		...
		$g->select_command = "SELECT * FROM (
								...
								,u.ip_address,u.email,u.created_on
		...
		$col["editable"] = true;
		$col["width"] = "150";
		$col["show"] = array("add"=>false,"edit"=>false,"list"=>false,"view"=>false);
		$cols[] = $col;
		...
		// $str = $g->get_dropdown_values("SELECT DISTINCT ID AS k, NAME AS v  FROM GROUPS WHERE ID <>0 ORDER BY ID");
		...
		$col["width"] = "130";
		$col["edittype"] = "select";
		$str = $g->get_dropdown_values("SELECT DISTINCT id AS k, NAME AS v  FROM groups WHERE id<>0 ORDER BY NAME");
		$col["editoptions"] = array("value" =>$str, "separator" => ":", "delimiter" => ";", "multiple" => true);
		$col["show"] = array("add"=>false,"edit"=>false,"view"=>false);
		$cols[] = $col;
		
		$col = array();
		$col["title"] = "Reff Acount PPK";
		$col["name"] = "iduser_ppk";
		$col["search"] = false;
		$col["export"] = false;
		$col["isnull"] = true;
		...
		$col["edittype"] = "select";
		$col["formatter"] = "select";
		$str = $g->get_dropdown_values("SELECT u.id AS k, CONCAT(first_name,' ',IFNULL(last_name,''),'-',UPPER(IFNULL(g.name,''))) AS v 
										FROM users u
										LEFT JOIN users_groups ug ON u.id=ug.user_id
										LEFT JOIN groups g ON g.id=ug.group_id
										where g.id=2
										 ");
		$col["editoptions"] = array("value" =>"NULL:;".$str, "separator" => ":", "delimiter" => ";" );
		$col["formatoptions"] = array("value" =>"NULL:;".$str, "separator" => ":", "delimiter" => ";" );
		$col["show"] = array("add"=>false,"edit"=>true,"view"=>true,"list"=>true);
		$cols[] = $col;
		...
		$col["export"] = false;
		$col["align"] = "center";
		$str1  = "<a href='".base_url()."auth/edit_user/{id}' target='_blank' class='ui-custom-icon ui-icon ui-icon-pencil' ></a>"; 
		// $str1 .= "&nbsp;<a href='".base_url()."kontrak/ai_update_lmp/dwi' target='_blank' class='btn btn-info' style='width:70px;height:30px;'>DWI</a>"; 
		// $str1 .= "&nbsp;<a href='".base_url()."kontrak/ai_update_lmp/mttrttr' target='_blank' class='btn btn-warning' style='width:100px;height:30px;'>MTTR&TTR</a>"; 
		$col["default"] = $str1;
		...
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["Management User","Pengguna"]);
	...
	public function create_user()
	{
		...
		else
		{
			...
			// $this->_render_page('layout' . DIRECTORY_SEPARATOR . 'header', $this->head);
			// $this->_render_page( 'create_user', $this->data);
			// $this->_render_page('layout' . DIRECTORY_SEPARATOR . 'footer');
			$this->load->view('layout/header',$head);
			$this->load->view('layout/sidebar');
	...
	*/
	...
	{		
		...
		$head['collapse'] = true; 
		$this->load->model('Global_model');
		$data['breadcrumbs'] = $this->Global_model->create_breadcrumbs(["Management User","Grup"]);
