 ...
 * @version 2.6.3 build 20190926-0259
...
class jqgrid
{
	...
	function __construct($db_conf = null)
	{
		...
		$grid["sortorder"] = "asc";
		$grid["form"]["nav"] = false;
		$grid["onPaging"] = "function (btn) { btn = btn.replace('_pager',''); btn = btn.substr(btn.indexOf('_')+1); $('#'+btn).closest('.ui-jqgrid-bdiv').scrollTop(0); }";
	...
	private function make_where($jsona)
	{
		...
		if ($search_type == "global")
		{
			...
			foreach($rules as $r)
			{
				...
				// use exact match "blah blah" if in quotes '
				if (preg_match("/'[^']+'/",$r["data"],$m))
					$data_arr = array(trim($r["data"],"'"));
				else
	...
	function set_options($options)
	{
		...
		{
			$this->options["shrinkToFit"]=false;
			unset($this->options["responsive"]);
		}
	...
	function set_columns($cols = null, $change_field = false)
	{
		...
		for($i=0,$j=0;$i<count($cols);$i++)
		{
			...
			if (!empty($this->options["cmTemplate"]["visible"]))
			...
			### P ###
			if ((is_array($cols[$i]["editoptions"]["onchange"]) || is_array($cols[$i]["editoptions"]["onload"])) && $cols[$i]["edittype"] == "select")
			{
				$col_tmp = $cols[$i];
		...
		if (isset($_POST['oper']))
		{
			...
			switch($op)
			{
				...
				case "add":
					...
					### apip nambah fungsi on_after_clone ###
		...
		if ($this->options["treeGrid"]==true)
		{
			...
			if ($this->options["treeConfig"]["loaded"] === false)
			{
				...
				else
				{
					...
					$where_tree = 'ISNULL('.$parent_col.') OR '.$parent_col.' = 0'; // roots
				}
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			if (isset($_GET["export"]))
			{
				...
				if ($this->options["export"]["format"] == "pdf")
				{
					...
					$pdf->SetCreator("tvri.ptmsp.id");
					$pdf->SetAuthor('tvri.ptmsp.id');
					$pdf->SetTitle('tvri.ptmsp.id');
					$pdf->SetSubject($this->options["caption"]);
					$pdf->SetKeywords('tvri.ptmsp.id');
					...
					if ($total_width==0 || $this->options["export"]["colwidth"] == "equal")
					{
		...
		foreach($this->options["colModel"] as &$c)
		{
			...
			if (!empty($c["link"]))
			{
				...
				$c["formatter"] = "function(cellval,options,rowdata){
					...
					var l = '".$c["link"]."';
					var tmp = $('#list1').getRowData(options.rowId);
					if (tmp) rowdata = tmp;
			...
			if (isset($c["editrules"]["readonly"]))
			{
				if ($c["editrules"]["readonly"] === true)
				{
					...
					if ($tag == "checkbox")
					{
						$str_inline_access .= 'jQuery("input[name='.$c["index"].'][type=checkbox]:last").attr("disabled","disabled");';
					}
					else
					{
		...
		$out .= ",'onSelectRow': function(ids) { ";
				...
				if (isset($this->options["detail_grid_id"]) && $this->options["detail_grid_id"] != '')
				{
					...
					foreach($d_grids as $detail_grid_id)
					{
						...
						{
							// if multiselect pass all ids to detail grid
							url_ids = ids;
							if (jQuery('#".$grid_id."').jqGrid('getGridParam','selarrrow').length)
								url_ids = jQuery('#".$grid_id."').jqGrid('getGridParam','selarrrow');

							jQuery('#".$detail_grid_id."').jqGrid('setGridParam',{datatype:'json',url:'".$detail_url."&rowid='+url_ids+data,editurl:'".$detail_url."&rowid='+ids+data,cellurl:'".$detail_url."&rowid='+ids+data,jqgrid_page:1});
							jQuery('#".$detail_grid_id."').trigger('reloadGrid',[{jqgrid_page:1}]);
						}
		...
		$out .= ",'loadComplete': function(ids) {";
				...
				$out .= "
					...
					if ( get_querystring_value('form') == 'add' || '$dialog' == 'add')
					{
						...
						{
							fx_show_form('{$grid_id}',false,true,$add_options);
						}
					}";
	...
	?>
		...
		<?php ### P ### ?>
		fx_show_form = function(grid_id, container, redirect, extra_opts)
		{
													...
													"change": function( event, ui )
															{
																...
																if (jQuery(window).data('validate_ac_'+el.name) == undefined) return;
							...
							else
							{
								...
								else
								{
									...
									if (!jQuery(sel).data('select2'))
										jQuery(sel).prepend("<option value='' selected='selected'>Select...</option>");
					...
					success: function (data, status)
					{
						...
						if(typeof(data.error) != 'undefined')
						{
							...
							else
							{
								...
								{
									var i = "<br><img onclick='window.open(\""+'<?=base_url()?>'+data.msg+"\")' style='cursor:hand;cursor:pointer;max-height:150px; max-width:90%' src='<?=base_url()?>"+data.msg+"'>";
			...
			jQuery(document).ready(function(){
				...
				jQuery("#<?php echo $grid_id ?>_toppager #<?php echo $grid_id ?>_pager_left")
								.append($("<table align='right' class='global-search ui-pg-table navtable' style='text-align: right; width: 25%' cellpadding='0' cellspacing='0' border='0'><tr><td>" +
										"<input autocomplete='off' placeholder='' id='<?php echo $grid_id ?>_globalsearchtext' style='padding:5px; width:80%; border: 1px solid #cdcdcd'></input>&nbsp;" +
		...
		fx_freeze_grid = function(id)
		{
			...
			var offset = 0;
			if ($(grid.grid.bDiv).get(0).scrollWidth > $(grid.grid.bDiv).width())
				var offset = 16;

			// fix for height pixel
			$(grid.grid.fbDiv).height($(grid.grid.bDiv).height()-offset);
		...
		normalize_height = function (grid_id)
		{
			...
			// sync top position
			$(grid.grid.fbDiv).css('top',$(grid.grid.bDiv).position().top);
			$(grid.grid.fhDiv).css('top',$(grid.grid.hDiv).position().top);

			// sync footer position
			if ($(grid.grid.sDiv).length)
			{
				$(grid.grid.fsDiv).css('top',$(grid.grid.sDiv).position().top);
				$(grid.grid.fsDiv).css('z-index','1');
			}
	...
	{
		// fix for d/m/Y or d/m/y date format. strtotime expects m/d/Y
		if (stristr($srcfmt,"d/m/Y"))
		{
			$val['data'] = preg_replace('/(\d+)\/(\d+)\/(\d+)/i','$2/$1/$3',$val['data']);
		}
		// fix for d-m-y (2 digit year) for strtotime
		else if (strstr($srcfmt,"d-m-y"))
		{
			$val['data'] = preg_replace('/(\d+)-(\d+)-(\d+)/i','$3-$2-$1',$val['data']);
		}
...
{
	/* apip add command validation error : 
	** Severity: Warning --> Invalid argument supplied for foreach() /home/ptmsp/public_html/devippl/application/libraries/phpgrid/jqgrid_dist.php 8111
	*/
	if(is_array($input))
